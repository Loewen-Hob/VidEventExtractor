...
def process_folders(input_txt_base_folder, input_srt_base_folder, output_base_folder):
    # 首先，计算总的文件数量
    total_files = sum([len(files) for _, _, files in os.walk(input_txt_base_folder) if any(file.endswith('.txt') for file in files)])
    
    # 使用 tqdm 创建一个进度条
    pbar = tqdm(total=total_files, desc="Processing files")
    
    for subdir, _, files in os.walk(input_txt_base_folder):
        for file in files:
            if file.endswith('.txt'):
                txt_file_path = os.path.join(subdir, file)
                srt_file_name = file.replace('.txt', '.srt')
                srt_subdir = subdir.replace("OCR", "ASR")
                srt_file_path = os.path.join(srt_subdir, srt_file_name)
                output_file_path = os.path.join(output_base_folder, os.path.relpath(subdir, input_txt_base_folder), file)
