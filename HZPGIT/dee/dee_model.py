...
class GITModel(nn.Module):
    ...
    def get_batch_sent_emb(self, ner_token_emb, ner_token_masks, valid_sent_num_list):
        # 从 [ner_batch_size, sent_len, hidden_size] 转换为 [ner_batch_size, hidden_size]
        if self.config.seq_reduce_type == 'AWA':
        ...
        # # 打印调试信息
        # print(f"total_sent_emb size: {total_sent_emb.size()}")
        # print(f"total_sent_pos_ids length: {len(total_sent_pos_ids)}")

        # 检查 total_sent_emb 和 total_sent_pos_ids 的维度是否匹配
        if total_sent_emb.size(0) != len(total_sent_pos_ids):
            raise ValueError(f"Dimension mismatch: total_sent_emb size {total_sent_emb.size(0)} vs. total_sent_pos_ids length {len(total_sent_pos_ids)}")

        total_sent_pos_ids = torch.tensor(total_sent_pos_ids, dtype=torch.long, device=total_sent_emb.device, requires_grad=False)
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False):
        ...
        if need_label_flag:
            ...
            ner_token_labels = torch.tensor([], dtype=torch.long)  # 初始化为空张量
        ...
        device = ner_token_ids[0].device  # 确定设备

        ner_token_ids = torch.cat(ner_token_ids, dim=0).to(device)
        ner_token_masks = torch.cat(ner_token_masks, dim=0).to(device)
        if need_label_flag:
            ner_token_labels = torch.cat(ner_token_labels, dim=0).to(device)
        else:
            # 如果没有标签，则初始化为适当的尺寸
            ner_token_labels = torch.zeros((ner_token_ids.size(0), ner_token_ids.size(1)), dtype=torch.long).to(device)

        hidden_size = 768  # 假设hidden_size为768，可以根据实际情况调整
        ner_token_emb = torch.zeros((ner_token_ids.size(0), ner_token_ids.size(1), hidden_size), dtype=torch.float).to(device)  # 初始化为全零张量
        ner_loss = None
        ner_token_preds = ner_token_labels  # 使用传入的真值作为NER输出

        ner_token_types = ner_token_preds
        
        # print(f"ner_token_preds: {ner_token_preds}")
        ...
        for valid_sent_num in valid_sent_num_list:
            ner_batch_idx_start_list.append(ner_batch_idx_start_list[-1] + valid_sent_num)
        ...
        # print(f"ner_token_labels: {ner_token_labels}")
    ...
    def forward(self, doc_batch_dict, doc_features,
        ...
        doc_features = [fea.to(device) if isinstance(fea, torch.Tensor) else fea for fea in doc_features]
        ...
        # print(f"doc_batch_dict: {doc_batch_dict}")
...
class SentencePosEncoder(nn.Module):
    ...
    def forward(self, batch_elem_emb, sent_pos_ids=None):
        ...
        # 打印调试信息
        # print(f"batch_elem_emb size: {batch_elem_emb.size()}")
        # print(f"batch_pos_emb size: {batch_pos_emb.size()}")

        if batch_elem_emb.size(0) != batch_pos_emb.size(0):
            raise ValueError(f"Dimension mismatch: batch_elem_emb size {batch_elem_emb.size(0)} vs. batch_pos_emb size {batch_pos_emb.size(0)}")
