...
class DEETask(BasePytorchTask):
    ...
    def resume_save_eval_at(self, epoch, resume_cpt_flag=False, save_cpt_flag=True):
        """
        在指定的epoch进行模型的保存、恢复和评估。

        参数:
        - epoch: 当前的训练轮次，用于保存和恢复模型，以及评估指标。
        - resume_cpt_flag: 是否恢复模型的状态。默认为False。
        - save_cpt_flag: 是否保存模型的状态。默认为True。
        """
        # 打印进度信息和当前的教师概率
        ...
        for task_idx, (data_type, gold_span_flag) in enumerate(eval_tasks):
            ...
            # 根据评估类型、是否使用金标span和当前epoch生成保存文件名
    ...
    def eval(self, features, dataset, use_gold_span=False,
        ...
        # 处理dump_decode_pkl_name, 用于存储解码结果
        ...
        # 处理dump_eval_json_name, 用于存储评估结果
