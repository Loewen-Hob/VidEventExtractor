...
class DEEExample(object):
        ...
        if only_inference:
            ...
            for mspan in self.ann_valid_mspans:
                for drange in self.ann_mspan2dranges[mspan]:
                    ...
                    extracted_span = sent_text[char_s:char_e].strip()

                    if extracted_span != mspan:
                        trimmed_char_e = char_e
                        while extracted_span != mspan and trimmed_char_e > char_s:
                            trimmed_char_e -= 1
                            extracted_span = sent_text[char_s:trimmed_char_e].strip()
            ...
            for mrecguid, event_name, event_dict in self.recguid_eventname_eventdict_list:
                print(f"Processing event name: {event_name}")  # 添加调试信息
                if event_name not in event_type2event_class:
                    raise KeyError(f"Event name '{event_name}' is not defined in event_type2event_class")
    ...
    def preprocess_data(detail_align_info):
        # 清理所有句子和标注中的多余空格和不可见字符
        detail_align_info['sentences'] = [sent.strip() for sent in detail_align_info['sentences']]
        detail_align_info['ann_valid_mspans'] = [mspan.strip() for mspan in detail_align_info['ann_valid_mspans']]
    ...
    def convert_dict_to_example(self, annguid, detail_align_info, only_inference=False):
        ...
        detail_align_info = preprocess_data(detail_align_info)
...
class DEEFeatureConverter(object):
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        event_arg_idxs_objs_list = []  # event_type_idx -> event_obj_idx -> event_arg_index -> span_idx
            ...
            if event_type not in dee_example.event_type2event_objs:
                ...
                for event_obj in event_objs:
                    ...
                    for field in event_fields:
                        ...
                        # 确保 arg_span 是字符串
                        if isinstance(arg_span, list):
                            arg_span = ' '.join(arg_span)
...
def print_total_eval_info(data_span_type2model_str2epoch_res_list,
    ...
    if (dev_type, span_type) not in data_span_type2model_str2epoch_res_list:
        raise KeyError(f"The key ('{dev_type}', '{span_type}') is missing in data_span_type2model_str2epoch_res_list.")
