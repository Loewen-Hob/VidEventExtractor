...
def remove_conflicts(annotations):
    """
    移除冲突的实体标注
    """
    ...
    for ann in annotations:
        conflict = False
        for res_ann in resolved_annotations:
            if not (ann[1][1] <= res_ann[1][0] or ann[1][0] >= res_ann[1][1]):
                conflict = True
                break
        if not conflict:
...
def preprocess_data(detail_align_info):
    ...
    # 获取ann_valid_dranges和ann_mspan2dranges的值
    ann_valid_dranges = detail_align_info['ann_valid_dranges']
    ann_mspan2dranges = detail_align_info['ann_mspan2dranges']

    # 移除ann_valid_dranges中的冲突
    resolved_ann_valid_dranges = remove_conflicts(ann_valid_dranges)
    detail_align_info['ann_valid_dranges'] = resolved_ann_valid_dranges

    # 移除ann_mspan2dranges中的冲突
    for mspan, dranges in ann_mspan2dranges.items():
        resolved_dranges = remove_conflicts(dranges)
        detail_align_info['ann_mspan2dranges'][mspan] = resolved_dranges
