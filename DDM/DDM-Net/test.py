...
parser.add_argument(
    "--resume",
    default="/root/bishe/DDM/checkpoint.pth.tar",
    type=str,
    metavar="PATH",
...
#         frame = path.split("/")[-1]
#         try:
#             frame_idx = int(frame[4:9])  # 尝试将帧名称的一部分转换为整数
#         except ValueError:
#             print(f"Skipping frame with invalid index: {frame}")
#             continue  # 如果转换失败，跳过这个帧并继续处理下一个
