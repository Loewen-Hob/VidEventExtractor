...
class TransformerDecoderLayer(nn.Module):
    ...
    #     if pos.size(0) > tensor.size(0):
    #         # 如果pos的序列长度大于tensor的序列长度，裁剪pos
    #         pos = pos[:tensor.size(0), :, :]
    #     elif pos.size(0) < tensor.size(0):
    #         # 如果pos的序列长度小于tensor的序列长度，需要扩展pos
    #         # 这里的扩展方法很简单，仅重复最后一个位置编码以填充到所需长度
    #         # 更复杂的策略可能需要根据具体情况设计
    #         extra_size = tensor.size(0) - pos.size(0)
    #         extra_pos = pos[-1:, :, :].repeat(extra_size, 1, 1)
    #         pos = torch.cat([pos, extra_pos], dim=0)
        
    #     # 现在pos的尺寸应该与tensor匹配，可以安全进行加法操作
    #     return tensor + pos
