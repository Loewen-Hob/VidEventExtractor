...
class KineticsGEBDMulFrames(Dataset):
    ...
    def _prepare_pickle(
        ...
        # dict_train_ann
        anno_path = '/root/bishe/DDM/data/k400_mr345_val_min_change_duration0.3.pkl'
        ...
        for vname in dict_train_ann.keys():
            ...
            # 修改
            root = '/root/bishe/DDM/data/kinetics_GEBD_RGB/val'
            video_dir = os.path.join(root, cls, frame_folder)
            if not os.path.exists(video_dir):
                continue


            vlen = min(vlen, len(os.listdir(video_dir)))
            ...
            for idx, (current_idx, lbl) in enumerate(zip(selected_indices, GT)):
                ...
                record = dict()
                shift = np.arange(-self.frame_per_side, self.frame_per_side + 1)
                shift[shift >= 0] += 1
                shift = shift * ds
                ...
                record["folder"] = f"{root}/{cls}/{frame_folder}"
